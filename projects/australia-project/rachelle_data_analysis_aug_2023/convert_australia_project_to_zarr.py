...
FILE_NAME_PATTERN = re.compile(r"^(?P<slide_id>TS[0-9]{2})\s(?P<well_id>well[1-9]+)\s?(?P<channel_name>[A-Z]*)\s?.tif$")

PRE_MALDI_CHANNELS = {"", "H", "GFP", "WGA"}
POST_MALDI_CHANNELS = {"PM"}
FLUORESCENCE_CHANNELS = {"H", "GFP", "WGA"}
...
def convert_australia_project_to_ngff_zarr(
    ...
    for (slide_id, well_id), group in groupby(tiff_files, key=extract_slide_well_id):
        ...
        post_maldi_paths, post_maldi_channel_names = zip(*[(p, n) for p, n in zip(well_tiff_files, channel_names) if n in PRE_MALDI_CHANNELS])
