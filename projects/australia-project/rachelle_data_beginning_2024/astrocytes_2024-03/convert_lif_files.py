...
def convert_lif_australia_file_to_ngff_zarr(
    ...
    pattern = r"Slide (\d+)/Region (\d+)_Merged"
    ...
    for image_idx, image in enumerate(lif_file.get_iter_image()):
        ...
        well_number = regions_to_wells[well_number]

        if slide_number not in ["4", "6"]:
            continue

        out_path = out_dir / f"slide{slide_number}/microscopy.zarr"
...
if __name__ == "__main__":
    ...
    convert_lif_australia_file_to_ngff_zarr(
        input_lif_file=Path("/scratch/bailoni/datasets/australia_project/new_data_feb_2024/astrocytes_march_24/astrocytes_datasets_premaldi_UOW.lif"),
        out_dir=Path("/scratch/bailoni/datasets/australia_project/new_data_feb_2024/astrocytes_march_24/spacem_v1_rachelle_premaldi"),
    )
